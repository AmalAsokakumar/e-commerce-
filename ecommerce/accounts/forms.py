...
class RegistrationForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        ).clean()  # super class is used to change the way this fields is being saved.
        password = cleaned_data.get("password")
        confirm_password = cleaned_data.get("confirm_password")
        if (
            password != confirm_password
        ):  # basically checks if both password matches or not
            raise forms.ValidationError("password does not match!")

    def __init__(
        self, *arg, **kwargs
    ):  # using a constructor am applying style to each field of the above form (applying class
